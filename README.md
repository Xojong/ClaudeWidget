...
│ ─────────────────────────────│
│ 19:09 (1:23 남음)  7D·Fbl 화 21:00 │   5시간 창 리셋 시각 · 남은 시간 │ 주간 리셋 요일·시각
╰──────────────────────────────╯
```

푸터 오른쪽 절반은 주간 창의 리셋 시각입니다. 7D와 Fbl은 실제로 같은 순간에 리셋되므로
평소엔 `7D·Fbl 화 21:00` 한 줄로 접어 표시하고, 두 시각이 다를 때만 `7D 화 21:00` /
`Fbl 수 09:00` 두 줄로 나뉩니다 (그때만 높이가 한 줄 늘어남). 폭은 게이지 행이 정하므로
푸터 때문에 위젯이 넓어지는 일은 없습니다. 영어 UI에서는 `Tue 21:00`.
